...
> - [ ] Install Node.js (v18.17+) and run `node --version` successfully.
>
...
- **Code example (run these in your terminal):**
  ...
  # Answer the prompts like this for this course:
...
- **Plain-English explanation:**
  By default, Next.js tries to make pages **static** — it fetches data at build
  time and caches it (this is the SSG strategy from Module 2). You control this
  per fetch with options, or per route with segment config:

  - **Static (default/cached):** the fetched data is reused; fast and cheap.
  - **Revalidated (ISR):** re-fetch at most every N seconds with
    `fetch(url, { next: { revalidate: 60 } })` or `export const revalidate = 60`.
  - **Dynamic (always fresh, SSR):** never cache with
    `fetch(url, { cache: "no-store" })` or `export const dynamic = "force-dynamic"`.

- **Code example:**

  ```jsx
  // Static (default): fetched once, cached.
  ...
  // Dynamic: fetch fresh on every single request.
...
> **COMMON MISTAKE TO AVOID**
> **Not realizing your page is static.** Beginners update data in their API and
> are baffled the site "won't update" — because the page was cached at build time.
> If data should change without a redeploy, you need `revalidate` (ISR) or
> `no-store` (dynamic). Knowing *which* rendering you're getting is essential.
...
| **SSG** | Static Site Generation — HTML built once at build time (the default). |
...
```jsx
// Static (default): cached, built once.
await fetch(url);
...
// Dynamic (SSR): always fresh, never cached.
